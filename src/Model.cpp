...
namespace Bodhisattva {
	Model::Model(glm::vec3* position, glm::quat* rotation, glm::vec3* scale, Material* mat)
		:position(position), rotation(rotation), scale(scale), material(mat)
	{
	}

	void Model::loadModel(std::string path, bool loadTextures)
	{
		...
		processNode(ActorList->mRootNode, ActorList, loadTextures);
	}

	void Model::processNode(aiNode * node, const aiScene * ActorList, bool loadTextures)
	{
		...
		for (unsigned int i = 0; i < node->mNumMeshes; i++)
		{
			...
			meshes.insert({ mesh->mName.C_Str(), processMesh(mesh, ActorList, loadTextures) });
		}
		...
		for (unsigned int i = 0; i < node->mNumChildren; i++)
		{
			...
			processNode(node->mChildren[i], ActorList, loadTextures);
		}
	}

	Mesh Model::processMesh(aiMesh * mesh, const aiScene * ActorList, bool loadTextures)
	{
			...
			if (thisMaterial != nullptr)
			{
				...
				if (Engine::getCurrentScene() != nullptr)
				{
					...
					if (currentLight != nullptr)
					{
						...
						glUniform3f(glGetUniformLocation(thisMaterial->shaderID, "lightPosition"), currentLight->position.x, currentLight->position.y, currentLight->position.z);
					}
				}
